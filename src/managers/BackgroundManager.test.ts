...
describe("BackgroundManager", () => {
  ...
  describe("launch", () => {
    it("should create a task with pending status", async () => {
      ...
      expect(status?.command).toBe("pwd");
    });
    ...
    it("should start tasks immediately when under concurrency limit", async () => {
      ...
      await new Promise((resolve) => setTimeout(resolve, 300));
       ...
       const status = await manager.getStatus(taskId);
       expect(status?.status).toBeOneOf(["running", "completed"]);
     });
  });
  ...
  describe("listTasks", () => {
    ...
    it("should return all tasks", async () => {
      await manager.launch({ command: "sh -c pwd" });
      await manager.launch({ command: "sh -c pwd" });
      await manager.launch({ command: "sh -c pwd" });
    ...
    it("should filter by status", async () => {
      const taskId = await manager.launch({ command: "sh -c pwd" });
